...
swisseph.swe_utc_time_zone(date.year, date.month, date.day, date.hour, date.minute, date.second, timeZone, function(re) {
  date = re;
  console.log('Date from Time Zone:', re, date);
});


var strtime = function (value) {
...
swisseph.swe_revjul(julday_ut, function(re) {
  console.log('swe_revjul:', re);
});

swisseph.swe_deltat(julday_ut,0, function(deltat) {
...
for (var i = 0; i < ephs.length; i++) {
  ...
  swisseph.swe_calc_ut(julday_ut, tagEph, flag, function(body) {
    console.log('%j position:%j , degree:%j', tag, body.longitude, toDegree(body.longitude));
    var out = {};
...
swisseph.swe_houses(julday_ut, geoLat, geoLon, 'P', function(result) {
  ...
  for (var i = 0; i < result.house.length; i++) {
    var degreeHouse = toDegree(result.house[i]);
    console.log('House:%j,%j,degree:%j', i, result.house[i], degreeHouse);
  }
});
console.log('========================================================');
